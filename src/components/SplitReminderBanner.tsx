        ...
        <div className="flex items-center justify-between gap-3 rounded-xl border border-primary/20 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent p-4 hover:border-primary/40 hover:from-primary/15 transition-all duration-300">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-primary/10 group-hover:bg-primary/20 transition-colors">
              {isRun ? <Footprints className="h-5 w-5 text-primary" /> : <Dumbbell className="h-5 w-5 text-primary" />}
            </div>
            <div className="text-left">
              ...
              <p className="text-xs text-muted-foreground">
                {isRun ? (
                  <>
                    {nextPreset.runData?.distanceKm ? `${nextPreset.runData.distanceKm} km` : 'Run'}
                    {nextPreset.runData?.durationSeconds ? ` — ${Math.floor(nextPreset.runData.durationSeconds / 60)}min target` : ''}
                    {' — tap to start'}
                  </>
                ) : (
